...
function getSupabase() {
  ...
  if (config.SUPABASE_URL && config.SUPABASE_SECRET_KEY) {
    try {
      supabaseClient = createClient(config.SUPABASE_URL, config.SUPABASE_SECRET_KEY, {
...
function mapOrderFromDb(o, items = []) {
  if (!o) return null;
  return {
    id: o.id,
...
function mapDynamicTableFromDb(dt) {
  if (!dt) return null;
  return {
    id: dt.id,
    title: dt.title,
